      ...
      <div className="md:hidden">
        <div className="bg-[#1a1d29] border border-[#2a2d3d] rounded-2xl p-4" onClick={privacy.toggle}>
          ...
          <div className="flex items-baseline gap-2 flex-wrap">
            <p className="text-3xl font-bold text-white font-mono">{pm(formatCurrency(nwResult.netWorth))}</p>
            {nwResult.monthlyChange !== 0 && (
              <span className={`text-sm font-medium font-mono ${nwResult.monthlyChange >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
          ...
          <div className="grid grid-cols-3 gap-2 mt-3">
            <div className="min-w-0">
              ...
              <p className="text-xs font-semibold text-emerald-400 font-mono truncate">{pm(formatCurrency(totalAssets))}</p>
            </div>
            <div className="min-w-0">
              <p className="text-[10px] text-gray-500">Liabilities</p>
              <p className="text-xs font-semibold text-rose-400 font-mono truncate">{pm(formatCurrency(totalLiabilities))}</p>
            </div>
            <div className="min-w-0">
              <p className="text-[10px] text-gray-500">Cash</p>
              <p className="text-xs font-semibold text-blue-400 font-mono truncate">{pm(formatCurrency(totalCash))}</p>
            </div>
          </div>
          {/* Proportion bar */}
            ...
            <Card>
              ...
              {(() => {
                ...
                return assetData.length > 0 ? (
                  <DonutChart data={assetData} height={240} colorMode="custom" emptyMessage="" />
                ) : <p className="text-gray-600 text-sm py-12 text-center">No asset accounts</p>
              })()}
            </Card>

            {/* Liabilities — unique color per account */}
            <Card>
              ...
              {(() => {
                ...
                const liabAccounts = accounts
                  ...
                  .map((a) => {
                    const color = a.type === 'credit' ? CREDIT_COLORS[ci++ % CREDIT_COLORS.length] : LOAN_COLORS[li++ % LOAN_COLORS.length]
                    return { ...a, bal: a.current_balance ?? 0, color }
                  })
                const liabTotal = liabAccounts.reduce((s, a) => s + a.bal, 0)
                    ...
                    <div className="mb-4">
                      ...
                      <div className="flex h-3 rounded-full overflow-hidden mt-2">
                        {liabAccounts.map((a, i) => {
                          ...
                          return <div key={a.id} className="h-full" style={{ width: `${pct}%`, backgroundColor: a.color }} />
                        })}
                      </div>
                      <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1.5">
                        {liabAccounts.map((a, i) => (
                          <div key={i} className="flex items-center gap-1">
                            ...
                            <span className="text-[10px] text-gray-500">{a.name.replace('Plaid ', '')}</span>
                          </div>
                        ))}
                      </div>
                    </div>
